"""
DIAGNOSTIC SCRIPT — trace the full pipeline lifecycle of a *single* named
candidate on a benchmark. Not part of the core solver handoff.
...
The target macro ID is parsed from the candidate name's ``mNNN`` token so the
script generalises to any benchmark/candidate pairing.
...
def main():
    ...
    _m = re.search(r"_m(\d+)_", target) or re.search(r"_m(\d+)$", target)
    target_macro_id = int(_m.group(1)) if _m else None
    print(f"\nLifecycle trace: '{target}' on {args.benchmark}")
    if target_macro_id is not None:
        print(f"  (parsed target macro_id={target_macro_id} from candidate name)")
    ...
    # --- Check pass 1: was the target macro a neighborhood seed candidate? ---
    print(f"\n[PASS 1] original_neighborhood candidates for macro {target_macro_id}:")
    m256_p1 = [sc for sc in ranked if sc.family == "original_neighborhood"
               and sc.metadata.get("moved_macro_id") == target_macro_id]
    if not m256_p1:
    ...
    m256_in_seeds = any(s.metadata.get("moved_macro_id") == target_macro_id for s in _seeds_used)
    print(f"\n  macro {target_macro_id} selected as refinement seed: {m256_in_seeds}")
    ...
    if not target_generated and m256_in_seeds:
        # Search for scale variants for the parsed target macro
        scale_tok = f"m{target_macro_id}"
        scale05 = [p for p in _ref_placements_generated if scale_tok in p.name and "scale" in p.name]
        print(f"  scale variants for {scale_tok} that WERE generated: {[p.name for p in scale05]}")
    ...
    if target_sc is None:
        ...
        scale_tok = f"m{target_macro_id}"
        close = [sc.name for sc in ranked if scale_tok in sc.name and "scale" in sc.name]
        print(f"  {scale_tok} scale variants in ranked: {close}")
    else:
